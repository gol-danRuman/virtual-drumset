    ...
    for key, value in upper.items():
        ...
        if key=='blue':
            ...
            if len(blue_cnts) > 0:
                ...
                ((x, y), blue_radius) = cv2.minEnclosingCircle(blue_c)
                ...
                if blue_radius > 0.5  and blue_radius < 40:
                    # draw the circle and centroid on the frame,
                    # then update the list of tracked points
                    cv2.circle(frame, (int(x), int(y)), int(blue_radius), colors[key], 2)
                    cv2.putText(frame, key + " ball", (int(x - blue_radius), int(y - blue_radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                                colors[key], 2)
            ...
            if len(red_cnts) > 0:
                ...
                ((x, y), red_radius) = cv2.minEnclosingCircle(red_c)

                M = cv2.moments(red_c)

                red_center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
                if red_radius > 0.5 and red_radius < 40:
                    # draw the circle and centroid on the frame,
                    # then update the list of tracked points
                    cv2.circle(frame, (int(x), int(y)), int(red_radius), colors[key], 2)

                    cv2.putText(frame, key + " ball", (int(x - red_radius), int(y - red_radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                                colors[key], 2)
